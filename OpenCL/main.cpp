/*
WARNING: Your GPU should support 16x16 work group size
*/
#include <stdio.h>
#include <stdlib.h>
...
int main(void) {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < K; j++) {
            a[i * K + j] = float(rand() % 10);
            //printf("a[%d][%d] = %f\n", i, j, a[i * K + j]);
        }
    }
    for (int i = 0; i < K; i++) {
        for (int j = 0; j < M; j++) {
            b[i * M + j] = float(rand() % 10);
            //printf("b[%d][%d] = %f\n", i, j, b[i * M + j]);
        }
    }
    float* ac = (float*)malloc(sizeof(float) * N * M);
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            c[i * M + j] = 0;
            ac[i * M + j] = 0;
    ...
    ret = clEnqueueWriteBuffer(command_queue, a_mem_obj, CL_TRUE, 0,
        sizeof(float) * N * K, a, 0, NULL, NULL);
    printf("ret at %d is %d\n", __LINE__, ret);

    ret = clEnqueueWriteBuffer(command_queue, b_mem_obj, CL_TRUE, 0,
        sizeof(float) * K * M, b, 0, NULL, NULL);
    printf("ret at %d is %d\n", __LINE__, ret);
    ...
    size_t* sizes = (size_t*)malloc(sizeof(size_t) * 3);
    ret = clGetDeviceInfo(device_id, CL_DEVICE_MAX_WORK_ITEM_SIZES, 1024, sizes, NULL);
    printf("ret at %d is %d\n", __LINE__, ret);

    size_t W = (TILE_W <= sizes[0]) ? TILE_W : sizes[0];
    size_t H = (TILE_H <= sizes[1]) ? TILE_H : sizes[1];
    ...
    printf("Total GFLOPS: %0.5f GFLOPS over 1439 GFLOPS\n", GFLOPS);
    // Read the memory buffer C on the device to the local variable C
    ret = clEnqueueReadBuffer(command_queue, c_mem_obj, CL_TRUE, 0,
        sizeof(float) * N * M, c, 0, NULL, NULL);
    printf("after copying\n");
    // Display the result to the screen
    bool is_successful = true;
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < M; j++) {
            //printf("c[%d][%d] = %f || %d\n", i, j, c[i * M + j], (c[i * M + j] == ac[i][j]));
            if (c[i * M + j] != ac[i * M + j]) {
                is_successful = false;
            }
        }
